...
# ---------------------------------------------------------
# 6. Define numeric columns for analysis
# ---------------------------------------------------------
numeric_cols = ["Cq", "Ampl.", "Slope"]

# Check if Block02_Phase06_Cycle00_GREEN exists and is numeric
...
if green_col in df.columns:
    ...
    df_ch2[green_col] = pd.to_numeric(df_ch2[green_col], errors="coerce")
    df_ch3[green_col] = pd.to_numeric(df_ch3[green_col], errors="coerce")

    # Check if there are any valid numeric values
    if df[green_col].notna().any():
        numeric_cols.append(green_col)
        print(f"\n✅ Added '{green_col}' to analysis (numeric column detected)")
    else:
        print(f"\n⚠️ '{green_col}' exists but contains no numeric values - skipping")
else:
    print(f"\n⚠️ Column '{green_col}' not found in data - skipping")

print(f"\nColumns being analyzed: {numeric_cols}")

# ---------------------------------------------------------
# 7. Aggregate CH2 and CH3 separately
# ---------------------------------------------------------
summary_ch2 = (
...
# ---------------------------------------------------------
# 8. Detection percentage (classification positive)
# ---------------------------------------------------------
detection = (
...
# ---------------------------------------------------------
# 9. Save to SAME Excel file (append new sheets)
# ---------------------------------------------------------
with pd.ExcelWriter(
        file_path,
        engine="openpyxl",
